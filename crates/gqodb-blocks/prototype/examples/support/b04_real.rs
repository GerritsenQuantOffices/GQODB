...
fn restore(layout: Layout, values: Vec<Vec<i64>>) -> Result<RecordBatch> {
    ensure!(
...
fn parquet_encode(batch: &RecordBatch, mode: Mode) -> Result<Vec<u8>> {
    ...
    for field in batch.schema().fields() {
        let encoding = match field.data_type() {
            DataType::Int64 => Encoding::DELTA_BINARY_PACKED,
...
pub fn encode(batch: &RecordBatch, mode: Mode) -> Result<Vec<u8>> {
    let mut out = match mode {
        Mode::Gqodb => {
            ...
            let payload = crate::adaptive::encode_values(&values, true, 0)?;
            let mut out = b"GQOREAL3".to_vec();
...
pub fn decode(input: Bytes, mode: Mode) -> Result<RecordBatch> {
    ...
    if matches!(mode, Mode::Gqodb) {
        ensure!(&input[..8] == b"GQOREAL3", "real block magic");
        ...
        let (kind, values) = crate::adaptive::decode_values(&input[12 + n..end])?;
