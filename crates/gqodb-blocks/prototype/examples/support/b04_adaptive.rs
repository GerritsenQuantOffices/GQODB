    ...
    while result.len() < rows {
        ...
        if width == 255 {
            ...
            for v in input[pos..pos + count * 8].chunks_exact(8) {
                ...
                result.push(q.wrapping_mul(scale).wrapping_add(base as u64) as i64);
            }
            pos += count * 8;
        } else {
            ...
            result.extend(
                integers[..count]
                    .iter()
                    .map(|&q| (q as u64).wrapping_mul(scale).wrapping_add(base as u64) as i64),
            );
            pos += bytes;
        }
    }
...
pub(crate) fn decode_values(input: &[u8]) -> Result<(u8, Vec<Vec<i64>>)> {
    ...
    for index in 0..cols {
        ...
        let packed = if compressed == 1 {
            uncompressed = lz4_flex::block::decompress(payload, expanded)?;
            &uncompressed[..]
        } else {
            payload
        };
        ensure!(packed.len() == expanded, "expanded length mismatch");
        let mut restored = unpack(packed, rows, base, scale)?;
        match predictor {
            Predictor::Absolute => {}
            Predictor::Delta => {
                ensure!(
                    restored.first().is_none_or(|&v| v == 0),
                    "invalid initial delta"
                );
                let mut previous = origin;
                for v in &mut restored {
                    previous = previous.wrapping_add(*v);
                    *v = previous;
                }
            }
            Predictor::Linear => {
                for (i, v) in restored.iter_mut().enumerate() {
                    *v = v
                        .wrapping_add(origin)
            ...
            Predictor::Reference => {
                for (v, r) in restored.iter_mut().zip(&values[reference]) {
                    *v = v.wrapping_add(*r);
                }
            }
        }
